...
class DefaultPlaceRepository : PlaceRepository {
    ...
    override suspend fun searchCurrentLocation(): Result<City,DataError.Local> {
        //returns an object which we need to map into a city
         ...
         is GeolocatorResult.Success -> {
          ...
          is GeolocatorResult.Error -> when(geoLocation){
              is GeolocatorResult.NotSupported -> {
                  Result.Error(DataError.Local.NOT_SUPPORTED)
              }
              is GeolocatorResult.PermissionError -> {
                  Result.Error(DataError.Local.PERMISSION_ERROR)
              }
              is GeolocatorResult.GeolocationFailed -> {
                  Result.Error(DataError.Local.GEOLOCATIONFAILED)
              }
    ...
    override suspend fun PlacesAutoComplete(query: String): Result<LocationAutoComplete, DataError.Remote> {
        ...
        return when (result) {
            is AutocompleteResult.Success -> {
                // Compass Autocomplete results are usually 'AutocompletePlace'
                // We map the list of results to our domain 'City' list
